...
const Cart = () => {
    const {products, cartItems, addToCart, removeFromCart, getCartCount, getCartAmount,currency, navigate,updateCartItem,} = useAppContext();
    ...
    return products.length > 0 && cartArray ? (
        <div className="flex flex-col md:flex-row py-16 ">
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button className="cursor-pointer mx-auto">
                            <img src={assets.remove_icon} alt="remove" className="inline-block w-6 h-6" onClick={()=>removeFromCart(product._id)}/>
                        </button>
                    </div>)
                )}
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {address.map((address, index)=> (
                                    <p onClick={() =>{setSelectedAdress(address); setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
                ...
                <button onClick={placeOrder} className="w-full py-3 mt-6 cursor-pointer bg-indigo-500 text-white font-medium hover:bg-indigo-600 transition">
                    {paymentOption === "COD" ? "Place an Order" : "Proceed to Checkout"}
                </button>
            </div>
        </div>
    ) : (null)
