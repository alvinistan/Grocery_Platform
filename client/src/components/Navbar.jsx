    ...
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 bg-white relative transition-all">
      ...
      <div className="hidden sm:flex items-center gap-8">
        ...
        <div className="hidden lg:flex items-center text-sm gap-2 border border-gray-300 px-3 rounded-full">
          ...
          />
          <img src={assets.search_icon} className="h-4 w-4" />
        </div>
        ...
        {!user ? (<button onClick={setUser(true)} className="cursor-pointer px-8 py-2 bg-indigo-500 hover:bg-primary-dull transition text-white rounded-full">
          Login
        </button>) : (
            <div className="relative group">
                <img src={assets.profile_icon} className="w-10" alt="user Icon"/>
                <ul className="hidden group-hover:block absolute top-10 right-0 bg-white shadow border border-gray-200 py-2.5 width-30 rounded-md text-sm z-40">
                    <li onClick={()=> navigate('/my-orders')} className="p-1.5 pl-3 hover:bg-primary/60 cursor-pointer">My Orders</li>
                    <li onClick={logout} className="p-1.5 pl-3 hover:bg-primary/60 cursor-pointer">Logout</li>
                </ul>
            </div>
        )}
      </div>
      ...
      >
        ...
        {!user ? (
          <button onClick={()=> {setOpen(false);setShowUserLogin(true);}} className="cursor-pointer px-6 py-2 mt-2 bg-indigo-500 hover:bg-primary-dull transition text-white rounded-full text-sm">
            Login
          </button>
        ) : (
